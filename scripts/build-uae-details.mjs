// Usage: node scripts/build-uae-details.mjs /tmp/sahaj-uae
// Reads <prefix>-<area>-osm.json exports. OSM contributors, ODbL.
// Geographic layers and landmark markers share one country projection.
import { readFile, writeFile } from 'node:fs/promises';
import { uaeMapPoint, uaeDetailBounds, uaeDetailExtents } from '../src/data/uae-map-projection.ts';
...
for (const area of Object.keys(uaeDetailExtents)) {
  ...
  const [left, top, width, height] = uaeDetailBounds(area);
  const tolerance = .003;
  ...
  for (const feature of source.elements) {
    ...
    // Join multipart water polygons if the supplied export includes them.
  ...
  svg += '</svg>\n';
  await writeFile(new URL(`../public/images/travel/uae-map/${area}-detail.svg`, import.meta.url), svg);
