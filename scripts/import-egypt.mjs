// Rebuild the selected Egypt photos. Source folders and metadata remain private.
...
const output = resolve(root, 'public/images/travel/egypt');
const selection = JSON.parse(await readFile(resolve(root, 'scripts/egypt-photo-selection.json'), 'utf8'));
const folders = ['egypt'];
...
for (const stop of selection) {
  ...
  for (const [index, photo] of stop.photos.entries()) {
    ...
    published[stop.slug].push({ src: `/images/travel/egypt/${id}.webp`, ...(index === 0 ? { thumbnail: `/images/travel/egypt/${id}-small.webp` } : {}), width: result.width, height: result.height, alt: photo.alt, capturedAt });
  }
}
await writeFile(resolve(root, 'src/data/egypt-photos.json'), JSON.stringify(published, null, 2)+'\n');
await mkdir(resolve(input, 'egypt-import'), { recursive: true });
await writeFile(resolve(input, 'egypt-import/metadata.json'), JSON.stringify(metadata, null, 2)+'\n');
